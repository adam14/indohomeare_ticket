...
<script>
	$(document).ready(function() {
		...
		var medic_tool_id = "<?php echo $medic_tool_contracts['medic_tool_id'] ?>";
		var medic_tool_session_id = "<?php echo $medic_tool_contracts['medic_tool_session_id']; ?>";
...
<?php $this->end(); ?>
<?php echo $this->Form->create(null, ['url' => ['action' => 'edit', $medic_tool_contracts['id']], 'type' => 'file', 'class' => 'form-horizontal', 'data-parsley-validate']); ?>
<fieldset>
		...
		<div class="col-lg-9">
            <select class="form-control" id="MedicTools" name="medic_tool_id" required>
                ...
                <?php foreach ($medic_tools as $value): ?>
                    <option value="<?php echo $value['id']; ?>" <?php echo ($medic_tool_contracts['medic_tool_id'] == $value['id']) ? 'selected' : ''; ?>><?php echo $value['name']; ?></option>
                <?php endforeach; ?>
            </select>
            <input name="contract_id" class="form-control" type="hidden" value="<?php echo $medic_tool_contracts['contract_id']; ?>">
		</div>
	</div>
	<div class="form-group">
